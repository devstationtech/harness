...
- **M3 — Multi-target emitters** ← next to specify. Emit `CLAUDE.md`, `.github/copilot-instructions.md`, `.cursor/rules` alongside `AGENTS.md`, driven by a `targets:` field in the manifest. Required because Claude Code does not read `AGENTS.md`. Open decision: build emitters in-house vs. delegate distribution to Ruler.
