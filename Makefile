...
# Pinned for reproducible CI: gofumpt is pinned to a release still compatible with
# the go directive in go.mod (newer gofumpt requires a newer Go than CI pins).
...
tools: ## Install dev tools (gofumpt, golangci-lint) into $(GOBIN)
	...
	@echo "installed gofumpt $(GOFUMPT_VERSION) and golangci-lint $(GOLANGCI_LINT_VERSION) into $(GOBIN)"
